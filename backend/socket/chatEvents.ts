  ...
  socket.on("getConversations", async () => {
    ...
    try {
      ...
      const conversations = await Conversation.find({
        ...
        .populate({
          path: "lastMessage",
          select: "Content senderId attachments createdAt",
        })
        .populate({
          path: "participants",
    ...
    try {
      if (data.type == "direct") {
        ...
        if (existingConversation) {
          ...
          return; // ← THÊM RETURN ĐỂ KHÔNG TẠO DUPLICATE
        }
      }
      ...
      const conversation = await Conversation.create({
        ...
        avatar: data.avatar || "", // Can be empty if direct conversation
        createdBy: socket.data.userId,
      });

      // Get all connectd sockets of all participants and make them join the room
      ...
      //Send conversation data back to all participants   (populated)
      ...
      // Emit conversation data to all participants
